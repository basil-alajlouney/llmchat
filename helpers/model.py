...
def add_role(role_name:str, role_desc:str) -> None:
  ...
  write_json_file(roles, os.path.join(Config.BASE_DIR, "store"), "roles.json")
...
def init_default_role():
  if not os.path.exists(os.path.join(Config.BASE_DIR, "store", "roles.json")):
    write_json_file({"helpful-assistant" : "you are a helpful assistant"}, os.path.join(Config.BASE_DIR, "store"), "roles.json")