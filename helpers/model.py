...
def add_role(role_name:str, role_desc:str) -> None:
  roles = read_json_file(Config.BASE_DIR + "/" + "store/roles.json")

  if role_name in roles.keys():
    raise ValueError(f"role already exists: {role_name}")
  
  roles[role_name] = role_desc
  write_json_file(roles, Config.BASE_DIR + "/" + "store", "roles.json")
...
def init_default_role():
  if not os.path.exists(Config.BASE_DIR + "/" + "store/roles.json"):
    write_json_file({"helpful-assistant" : "you are a helpful assistant"}, Config.BASE_DIR + "/" + "store", "roles.json")