...
from helpers.cli_formater import conversate, list_chats, list_colors, init_messages, validate_colors

import argparse
import importlib.metadata

from helpers.utils import read_json_file, verbose_print_init, write_json_file
...
def main():
  ...
  # Preparing the chat
  post_response_fn = lambda : write_json_file(messages, "store/chats", args.name + ".json")
  ...
  if len(messages) == 0:
    ...
    args.role = next((k for k, v in read_json_file("store/roles.json").items() if v == system_messages), None)
